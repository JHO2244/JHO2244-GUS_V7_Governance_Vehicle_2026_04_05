"""
GUS v7 — Phase 27
Replay Verification System Tests (v0.1)

STRICT:
- Replay must reproduce identical canonical envelope
- Any deviation must fail
- Fail-closed on invalid baseline or replay input
"""
...
from gus_v7.routing.execution_consistency_lock_v0_1 import (
    verify_replayed_execution_output_v0_1,
...
def test_phase27_replay_single_case_matches_baseline() -> None:
    ...
    replay_output = route_and_validate_case(case)

    assert verify_replayed_execution_output_v0_1(envelope, replay_output) is True
    assert case == baseline


def test_phase27_replay_batch_matches_baseline() -> None:
    ...
    replay_output = evaluate_case_batch_v0_1(batch)

    assert verify_replayed_execution_output_v0_1(envelope, replay_output) is True
    assert batch == baseline


def test_phase27_replay_cross_case_matches_baseline() -> None:
    batch = (_build_bc01_case(), _build_bc02_case(), _build_bc03_case())
    baseline = deepcopy(batch)
    ...
    replay_report = analyze_case_batch_v0_1(batch)

    assert verify_replayed_execution_output_v0_1(envelope, replay_report) is True
    assert batch == baseline


def test_phase27_replay_fails_on_different_output() -> None:
    envelope = wrap_canonical_execution_output_v0_1(("PASS", "FAIL"))

    replay_output = ("PASS", "PASS")

    assert verify_replayed_execution_output_v0_1(envelope, replay_output) is False


def test_phase27_replay_fails_on_invalid_baseline() -> None:
    assert verify_replayed_execution_output_v0_1({}, "PASS") is False


def test_phase27_replay_fails_on_invalid_replay_output() -> None:
    envelope = wrap_canonical_execution_output_v0_1("PASS")

    assert verify_replayed_execution_output_v0_1(envelope, ["PASS"]) is False

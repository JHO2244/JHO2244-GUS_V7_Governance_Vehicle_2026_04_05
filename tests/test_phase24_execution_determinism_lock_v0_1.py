"""
GUS v7 — Phase 24
Execution Determinism Lock Tests (v0.1)

STRICT:
- Same input -> same output
- No mutation of execution inputs
...
def test_phase24_single_case_execution_is_repeatable_and_non_mutating() -> None:
    case = _build_bc01_case()
    baseline = deepcopy(case)

    result_1 = route_and_validate_case(case)
    result_2 = route_and_validate_case(case)

    assert result_1 == result_2
    assert case == baseline


def test_phase24_batch_execution_is_repeatable_and_non_mutating() -> None:
    batch = (_build_bc01_case(), _build_bc02_case(), _build_bc03_case())
    baseline = deepcopy(batch)

    result_1 = evaluate_case_batch_v0_1(batch)
    result_2 = evaluate_case_batch_v0_1(batch)

    assert result_1 == result_2
    assert batch == baseline


def test_phase24_cross_case_analysis_is_repeatable_and_non_mutating() -> None:
    batch = (_build_bc01_case(), _build_bc02_case(), _build_bc03_case())
    baseline = deepcopy(batch)

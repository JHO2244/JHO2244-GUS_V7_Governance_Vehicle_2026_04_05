"""
GUS v7 — Phase 14
Batch Evaluation Engine Tests (v0.1)
...
def test_phase14_batch_evaluates_mixed_cases_in_order():
    batch = (_build_bc01_case(), _build_bc02_case(), _build_bc03_case())
    assert evaluate_case_batch_v0_1(batch) == (PASS, FAIL, INSUFFICIENT_EVIDENCE)


def test_phase14_batch_preserves_input_order():
    batch = (_build_bc03_case(), _build_bc01_case(), _build_bc02_case())
    assert evaluate_case_batch_v0_1(batch) == (INSUFFICIENT_EVIDENCE, PASS, FAIL)


def test_phase14_batch_invalid_container_fails_closed():
    assert evaluate_case_batch_v0_1("not_a_batch") == (FAIL,)


def test_phase14_batch_invalid_case_member_fails_only_that_case():
    batch = (_build_bc01_case(), "not_a_case", _build_bc03_case())
    assert evaluate_case_batch_v0_1(batch) == (PASS, FAIL, INSUFFICIENT_EVIDENCE)


def test_phase14_batch_empty_batch_returns_empty_tuple():
    assert evaluate_case_batch_v0_1(()) == ()
